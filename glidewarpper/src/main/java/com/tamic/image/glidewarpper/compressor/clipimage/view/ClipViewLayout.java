package com.tamic.image.glidewarpper.compressor.clipimage.view;
...
public class ClipViewLayout extends RelativeLayout {
    ...
    public Bitmap clip() {
        ...
        try {
            ...
            zoomedCropBitmap = zoomBitmap(cropBitmap, 400, 400);
        } catch (Exception e) {
            e.printStackTrace();
        }
        if (cropBitmap != null) {
     ...
     * @param requestWidth  期望的宽
     * @param requestHeight 期望的高
     * @return
     */
    public static Bitmap decodeSampledBitmap(String imagePath, int requestWidth,
                                             int requestHeight) {
        if (!TextUtils.isEmpty(imagePath)) {
            Log.d("--->", "requestWidth: " + requestWidth);
            Log.d("--->", "requestHeight: " + requestHeight);
            ...
            Log.d("--->", "original height: " + options.outHeight);
            Log.d("--->", "original width: " + options.outWidth);
            if (options.outHeight == -1 || options.outWidth == -1) {
                try {
                    ExifInterface exifInterface = new ExifInterface(imagePath);
                    int height = exifInterface.getAttributeInt(ExifInterface.TAG_IMAGE_LENGTH, ExifInterface.ORIENTATION_NORMAL);//获取图片的高度
                    int width = exifInterface.getAttributeInt(ExifInterface.TAG_IMAGE_WIDTH, ExifInterface.ORIENTATION_NORMAL);//获取图片的宽度
                    Log.d("--->", "exif height: " + height);
                    Log.d("--->", "exif width: " + width);
    ...
    public static int calculateInSampleSize(BitmapFactory.Options options,
        ...
        // Raw height and width of image_load_err
        ...
        if (height > reqHeight || width > reqWidth) {
            ...
            // guarantee
            // a final image_load_err with both dimensions larger than or equal to the
