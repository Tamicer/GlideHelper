...
public class ClipView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (clipType == ClipType.CIRCLE) {
            //中间的透明的圆
            canvas.drawCircle(this.getWidth() / 2, this.getHeight() / 2, clipRadiusWidth - 60, paint);
            //白色的圆边框
            canvas.drawCircle(this.getWidth() / 2, this.getHeight() / 2, clipRadiusWidth - 60, borderPaint);
        } else if (clipType == ClipType.RECTANGLE) { //绘制矩形裁剪框
            //绘制中间的矩形
            canvas.drawRect(mHorizontalPadding + 60, this.getHeight() / 2 - clipWidth / 2,
                    this.getWidth() - mHorizontalPadding - 60, this.getHeight() / 2 + clipWidth / 2, paint);
            //绘制白色的矩形边框
            canvas.drawRect(mHorizontalPadding + 60, this.getHeight() / 2 - clipWidth / 2,
                    this.getWidth() - mHorizontalPadding - 60, this.getHeight() / 2 + clipWidth / 2, borderPaint);
        }
